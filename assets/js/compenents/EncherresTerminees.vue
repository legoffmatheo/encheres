<template>
    <div class="encheres-terminees">
      <h2>Enchères terminées récemment</h2>
      <div class="liste-encheres" v-if="encheres.length > 0">
        <div class="carte-enchere" v-for="enchere in encheres" :key="enchere.id">
          ...
          <p><strong>Date de début :</strong> {{ formaterDate(enchere.dateHeureDebut) }}</p>
          <p><strong>Date de fin :</strong> {{ formaterDate(enchere.dateHeureFin) }}</p>
        </div>
      </div>
      <p v-else>Aucune enchère terminée récemment.</p>
    </div>
  </template>
  ...
  export default {
    setup() {
      ...
      // Fonction pour récupérer les enchères terminées
      const recupererEncheres = async () => {
        try {
          const reponse = await fetch("/api/encheres-terminees-recemment");
          if (reponse.ok) {
